...
const checapalavrasspam = (usermensagem) => {
    const palavraspam = ['bom dia', 'vender', 'vendo', 'gostaria', 'comprar', 'boa tarde', 'boa noite', 'acessar', 'clicar', 'comprar', 'R$', 'http']
    let i = 0
    let wordcount = 0
...
const startuserverification = (client, budy, from, mek, sender, palavroes, reply) => {
    ...
    if (usermensagem.length > 600) {
        if (usermensagem.length > 4500) {
            reply('-1000 pts Por Spam. Banido.')
        } else {
            const quantidadedepalavraspam = checapalavrasspam(usermensagem)
            console.log(usermensagem.length)
            console.log(quantidadedepalavraspam)
            if (quantidadedepalavraspam > 1) {
                reply('-100 pts Por Spam.')
            }
        }
    }
}
