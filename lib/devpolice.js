...
const cadastrar = async (client, from, groupMembers, usersjson) => {
    ...
    let jsonusers = usersjson.length
    let groupjsonusers = groupMembers.length

    let i = 0;

    while (i != groupjsonusers) {
        ...
        if (usersjson[i] == undefined) {
            ...
            if (usersjson[i]['numero'] == groupMembers[i].jid) {
                ...
                if (jatem == 0) {
                    await usersjson.push(person)
                    console.log(i + ': TÁ DEMORANDO: ' + groupMembers[i].jid)
...
const add = (isGroup, isOwner, mess, isGroupAdmins, client, from, isBotGroupAdmins, args, isQuotedVcard, isQuotedMessage1, mek, reply) => {
    if (verifiuser(isOwner, isGroup, isGroupAdmins, isBotGroupAdmins, reply)) {
        client.updatePresence(from, Presence.composing)
        if (isQuotedVcard) {
...
const kick = (isGroup, mess, isOwner, isGroupAdmins, client, from, isBotGroupAdmins, isQuotedMessage1, mek, mentions, text, reply) => {
    if (verifiuser(isOwner, isGroup, isGroupAdmins, isBotGroupAdmins, reply)) {
        ...
        if (isQuotedMessage1) {
            ...
            client.sendMessage(from, 'jae', text)
            let numbertoadd = []
