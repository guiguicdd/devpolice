...
const gravarpontos = (pon, unum, motivo, usersjson) => {
    let i = 0;
    while (i != usersjson.length) {
        if (usersjson[i].numero == unum) {
            pont = usersjson[i].pontos - pon
            mot = usersjson[i].motivos.push(motivo)
...
const startuserverification = async (client, budy, from, mek, sender, palavroes, usersjson, reply) => {
    ...
    if (usermensagem.includes('http://')) {
        ...
        reply(`🚔\n-${mpts}pts\nMotivo:\n${motivo}`)
        gravarpontos(mpts, usernumero, motivo, usersjson)
    }
