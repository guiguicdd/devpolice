...
const procuraxingamento = (usermensagem) => {
    console.log('1>')
    const badwords = ['cusao', 'viado', 'palavrao', 'palavrão', '55357']

    let i = 0
    while (i != badwords.length) {
        console.log(usermensagem)
        console.log(badwords[i])
        console.log(usermensagem.indexOf(badwords[i]))
        if (usermensagem.includes(badwords[i])) {
            console.log('ui')
            return true
        }
        i++
...
const startuserverification = (client, budy, from, mek, sender, isUrl, reply) => {
    ...
    if (procuraxingamento(usermensagem)) {
        console.log('manda o ban')
        reply('-100 pts Por desrespeito.')
    }


    console.log(
        usermensagem.codePointAt(0)
    );

    console.log(usernumero)
    console.log(utf8.decode(usermensagem))
    console.log(utf8.encode(usermensagem))
}
