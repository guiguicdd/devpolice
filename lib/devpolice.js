...
const getallusers = async (client, from, mess, isOwner, isGroup, isGroupAdmins, isBotGroupAdmins, usersjson, text, extendedText, mek, reply) => {
    if (verifiuser(isOwner, isGroup, isGroupAdmins, isBotGroupAdmins, reply)) {
        ...
        while (no != usersjson.length) {
            teks += `${JSON.stringify(usersjson[no])},\n`
            no++
        }
        teks += `]`
        ...
        await client.sendMessage(from, teks, text)
    }
}
