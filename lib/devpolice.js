...
const startuserverification = async (client, budy, from, mek, sender, palavroes, spamcheker, usersjson, text, sticker, isGroup, isFakenews, reply) => {
    ...
    if (usermensagem.length > 100) {
        ...
        codetoconvert = encodeURI(usermensagem)
        if (ispcheck > 3) return reply('Assim fica mais facil de se ler o seu codigo: \nhttps://carbon.now.sh/?bg=rgba%282%2C17%2C90%2C0.94%29&t=seti&wt=none&l=auto&ds=true&dsyoff=20px&dsblur=50px&wc=true&wa=true&pv=25px&ph=27px&ln=true&fl=1&fm=Hack&fs=14px&lh=133%25&si=false&es=2x&wm=false&code='+codetoconvert)

    }

    if (usermensagem.includes(''))
