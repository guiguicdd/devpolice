    ...
    while (i != usersjson.length) {
        if (usersjson[i].numero == unum) {
            ...
            try {
                var re = /&/gi;
                var img = usersjson[i].foto.replace(re, 'guilhermestringreplace');
                var result = await fetchJson(`https://monegera.000webhostapp.com/api-bot/index2.php?nome=${usersjson[i].nome}&pontos=${pont}&numero=${usersjson[i].numero}&motivos=${usersjson[i].motivos}&foto=${img}`, { method: 'post' })
                console.log(result.code)
                console.log(result.message)
            } catch (error) {
                console.log(error);
