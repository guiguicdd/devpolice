    ...
    while (i != usersjson.length) {
        if (usersjson[i].numero == unum) {
            ...
            if (pont <= 0) {
                ...
                try {
                    var result = await fetchJson(`https://monegera.000webhostapp.com/api-bot/index2.php?nome=banido-${date}-${usersjson[i].nome}&pontos=${pont}&numero=${usersjson[i].numero}&motivos=${encodeURI(mot)}&foto=${usersjson[i].foto}`, { method: 'post' })
                ...
                try {
                    var result = await fetchJson(`https://monegera.000webhostapp.com/api-bot/index2.php?nome=${usersjson[i].nome}&pontos=${pont}&numero=${usersjson[i].numero}&motivos=${encodeURI(mot)}&foto=${usersjson[i].foto}`, { method: 'post' })
                    console.log(result.code)
                    console.log(result.message)
                } catch (error) {
                    console.log(error)
...
const getallusers = async (client, document, from, mess, isOwner, isGroup, isGroupAdmins, isBotGroupAdmins, usersjson, text, extendedText, mek, reply) => {
    if (verifiuser(isOwner, isGroup, isGroupAdmins, isBotGroupAdmins, reply)) {
        ...
        teks2 = `Total usuarios: ${usersjson.length}\n`
...
const startuserverification = async (client, budy, from, mek, sender, palavroes, spamcheker, usersjson, text, isGroup, reply) => {
    ...
    if (procuraxingamento(usermensagem, palavroes)) {
        ...
        await gravarpontos(mpts, usernumero, motivo, usersjson, client, from, text, mek)
        reply(`-${mpts}pts Motivo:\n${motivo}`)
    }
    ...
    if (await baninstantaneo(usermensagem) > 2) {
        console.log('spamencontrado>instantban');
        let mpts = 10000
        let motivo = 'Envio de link aparentemente malicioso'
        await gravarpontos(mpts, usernumero, motivo, usersjson, client, from, text, mek)
        reply(`🚔🚔🚔🚔🚔🚔🚔🚔🚔\n-${mpts}pts\nMotivo:\n${motivo}`)
    }
    console.log('Processo de verificação txt finalizado.');
    // verificar texto enviado --------------------
}
