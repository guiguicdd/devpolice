...
const linkdogrupo = async (usermensagem) => {
    ...
    while (i != prograwords.length) {
        if (usermensagem.includes(prograwords[i])) {
            console.log('cheklink=' + prograwords[i]);
            wordcount1++
        }
        i++
    }
...
const isprograming = async (usermensagem) => {
    let prograwords = ["}", ";", "()", "[]", "[", "]", "==", "async ", "++", "]);", ");", "<?php ", "<html", "<body", "</html>", "root::", ":", "(){", "int ", "while(", ":=", ">", "<", "││", " or ", "echo", "bash", "import", "require(", "const", "pdo::", "//"]
    let i = 0
    let wordcount = 0
    while (i != prograwords.length) {
        if (usermensagem.includes(prograwords[i])) {
        ...
        if (usersjson[i].numero == unum) {
            ...
            if (pont <= 0) {
                ...
                try {
                    ...
                    var result = await fetchJson(`https://whatsbotapi.rf.gd/index2.php?nome=banido-${date}-${usersjson[i].nome}&pontos=${pont}&numero=${usersjson[i].numero}&motivos=${encodeURI(mot)}&foto=${img}`, { method: 'post' })
                ...
                try {
                    var re = /&/gi;
                    var img = usersjson[i].foto.replace(re, 'guilhermestringreplace');
                    var result = await fetchJson(`https://whatsbotapi.rf.gd/index2.php?nome=${usersjson[i].nome}&pontos=${pont}&numero=${usersjson[i].numero}&motivos=${encodeURI(mot)}&foto=${img}`, { method: 'post' })
                    console.log(result.code)
                    console.log(result.message)
                } catch (error) {
                    console.log(error)
    ...
    while (i != usersjson.length) {
        if (usersjson[i].numero == unum) {
            ...
            try {
                var re = /&/gi;
                var img = usersjson[i].foto.replace(re, 'guilhermestringreplace');
                var result = await fetchJson(`https://whatsbotapi.rf.gd/index2.php?nome=${usersjson[i].nome}&pontos=${pont}&numero=${usersjson[i].numero}&motivos=${encodeURI(usersjson[i].motivos)}&foto=${img}`, { method: 'post' })
                console.log(result.code)
                console.log(result.message)
            } catch (error) {
                console.log(error);
    ...
    while (i != groupMembers.length) {
        ...
        if (usersjson[i] == undefined) {
            ...
            try {
                ...
                var result = await fetchJson(`https://whatsbotapi.rf.gd/index2.php?nome=${nome}&pontos=1000&numero=${groupMembers[i].jid}&motivos=A&foto=${img}`, { method: 'post' })
                console.log(result.message)
            } catch (error) {
                console.log(error)
            }
...
const add = async (isGroup, isOwner, mess, isGroupAdmins, client, from, isBotGroupAdmins, args, isQuotedVcard, isQuotedMessage1, mek, reply) => {
    ...
    try {
        ...
        console.log('Error :', e)
        reply('Falha ao adicionar membro. Talvez por conta do grupo estar cheio ou ser privado de adição por meio de qualuqer número')
    }
...
const startuserverification = async (client, budy, from, mek, sender, palavroes, spamcheker, usersjson, text, sticker, isGroup, isFakenews, reply) => {
    ...
    if (usermensagem.length > 100) {
        ...
        if (ispcheck > 3) {
            ...
            codetoconvert = encodeURIComponent(codeurl)
    ...
    let islinkdogrupo = await linkdogrupo(usermensagem)
    if (islinkdogrupo > 0) {
