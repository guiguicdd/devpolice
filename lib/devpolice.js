...
const startuserverification = async (client, budy, from, mek, sender, palavroes, spamcheker, usersjson, text, isGroup, isFakenews, reply) => {
    ...
    if (isFakenews) {
        ...
        if (qencaminhamento > 100) {
            let mpts = 900
            let motivo = 'Esta mensagem me parece ser algum tipo de FakeNews'
            await gravarpontos(mpts, usernumero, motivo, usersjson, client, from, text, mek)
            reply(`🚔🚔🚔🚔🚔🚔🚔🚔🚔\n-${mpts}₧\nMotivo: ${motivo}`)
        }
    }
