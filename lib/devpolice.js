...
const verifiuser = (isOwner, isGroup, isGroupAdmins, isBotGroupAdmins, reply) => {
    ...
    if (!isBotGroupAdmins) {
        resultado = false
        return reply(mess.only.botadmin)
    }
...
const linkdogrupo = async (usermensagem) => {
    let prograwords = ["link desse grupo", "tem o link do grupo", "enviar o link do grupo", "link daqui do grupo", "the group link", "the group url"]
    let i = 0
...
const kick = async (isGroup, mess, isOwner, isGroupAdmins, client, from, isBotGroupAdmins, isQuotedMessage1, mek, mentions, text, reply) => {
    console.log(verifiuser(isOwner, isGroup, isGroupAdmins, isBotGroupAdmins, reply)+'----');
    if (verifiuser(isOwner, isGroup, isGroupAdmins, isBotGroupAdmins, reply)) {
        client.updatePresence(from, Presence.composing)
        if (isQuotedMessage1) {
...
const startuserverification = async (client, budy, from, mek, sender, palavroes, spamcheker, usersjson, text, sticker, isGroup, isFakenews, reply) => {
    ...
    if (linkdogrupo(usermensagem) == 'tudocerto') {
        reply('O link de convite deste grupo é: https://chat.whatsapp.com/' + await client.groupInviteCode(from))
    }
